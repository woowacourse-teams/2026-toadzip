...
@Entity
@Table(
        name = "lh_notice_collection_checkpoints",
        uniqueConstraints = @UniqueConstraint(columnNames = {"source", "request_hash"})
)
@NoArgsConstructor(access = PROTECTED)
public class LhNoticeCollectionCheckpoint {
    ...
    private String sourceNoticeKey;
    ...
    private LhNoticeCollectionCheckpoint(
        ...
        validateNotBlank(sourceNoticeKey, "원천 공고 식별자");
        ...
        this.source = source;
        this.sourceNoticeKey = sourceNoticeKey.strip();
    ...
    public static LhNoticeCollectionCheckpoint complete(
            ExternalDataSource source,
            String sourceNoticeKey,
            String requestDescription,
            String panId,
        ...
        return new LhNoticeCollectionCheckpoint(
                source,
                sourceNoticeKey,
                requestDescription,
                panId,
    ...
    private static void validateSource(ExternalDataSource source) {
        if (source != ExternalDataSource.LH_NOTICE_DETAIL && source != ExternalDataSource.LH_NOTICE_SUPPLY) {
            throw new IllegalArgumentException("LH 공고 상세·공급 원천만 체크포인트를 저장할 수 있습니다.");
        }
    }
