...
@Component
public class LhNoticeSourceMapper {

    public List<LhNoticeDetailSource> details(String panId, JsonNode root) {
        List<LhNoticeDetailSource> sources = new ArrayList<>();
        ...
        addNoticeFiles(sources, panId, root);
        addComplexImages(sources, panId, root);
        return sources;
    }

    public List<LhNoticeSupplySource> supplies(String panId, JsonNode root) {
        List<JsonNode> rows = DataGoKrOpenApiClient.findRows(root, "dsList01");
        List<LhNoticeSupplySource> sources = new ArrayList<>();
        for (int sourceOrder = 0; sourceOrder < rows.size(); sourceOrder++) {
            sources.add(new LhNoticeSupplySource(
                    sourceOrder,
                    panId,
                    LhNoticeSupplySourceItem.from(rows.get(sourceOrder))
            ));
        }
        return sources;
    }

    private void addEtcInfo(List<LhNoticeDetailSource> sources, String panId, JsonNode root) {
    ...
    private void addComplexes(List<LhNoticeDetailSource> sources, String panId, JsonNode root) {
    ...
    private void addSchedules(List<LhNoticeDetailSource> sources, String panId, JsonNode root) {
    ...
    private void addReceptions(List<LhNoticeDetailSource> sources, String panId, JsonNode root) {
    ...
    private void addNoticeFiles(List<LhNoticeDetailSource> sources, String panId, JsonNode root) {
        for (JsonNode row : DataGoKrOpenApiClient.findRows(root, "dsAhflInfo")) {
            sources.add(detail(sources.size(), panId, "NOTICE_FILE")
    ...
    private void addComplexImages(List<LhNoticeDetailSource> sources, String panId, JsonNode root) {
    ...
    private static final class DetailBuilder {
        ...
        private LhNoticeDetailSource build() {
            return new LhNoticeDetailSource(
