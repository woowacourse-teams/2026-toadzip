...
@Slf4j
@Repository
public class MyHomeComplexMappingExecutionLock {

    private static final long LOCK_KEY = 8_432_026_082_400_003L;
    ...
    public MyHomeComplexMappingExecutionLock(DataSource dataSource) {
    ...
    public <T> Optional<T> tryRun(Supplier<T> operation) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            throw new IllegalStateException("마이홈 단지 매핑 실행 잠금을 처리하지 못했습니다.", exception);
        }
        finally {
            localLock.unlock();
        }
    ...
    private void releaseDatabaseLock(Connection connection) {
        try {
            ...
            log.error(
                    "마이홈 단지 매핑 실행 잠금 해제에 실패했습니다. DB 연결 종료 시 잠금이 해제됩니다.",
                    exception
            );
        }
    }
