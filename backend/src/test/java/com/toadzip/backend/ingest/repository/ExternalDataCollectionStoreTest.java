package com.toadzip.backend.ingest.repository;

import static org.assertj.core.api.Assertions.assertThat;

import java.time.Instant;
import java.util.List;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.data.jpa.test.autoconfigure.DataJpaTest;

import com.toadzip.backend.ingest.domain.ExternalDataCollectionFailure;
import com.toadzip.backend.ingest.domain.ExternalDataSnapshot;
import com.toadzip.backend.ingest.domain.ExternalDataSource;

@DataJpaTest
class ExternalDataCollectionStoreTest {

    @Autowired
    private ExternalDataSnapshotRepository snapshotRepository;

    @Autowired
    private ExternalDataCollectionFailureRepository failureRepository;

    private ExternalDataCollectionStore store;

    @BeforeEach
    void setUp() {
        store = new ExternalDataCollectionStore(snapshotRepository, failureRepository);
    }

    @Test
    @DisplayName("외부 응답 원문과 수집 시각을 변경 없이 저장한다")
    void storesRawPayloadAndCollectedAt() {
        Instant collectedAt = Instant.parse("2026-08-23T00:00:00Z");
        String payload = "{\"response\": { \"body\": { \"item\": [] } }}";

        store.storeSnapshots(List.of(ExternalDataSnapshot.create(
                ExternalDataSource.MYHOME_COMPLEX,
                "rentalHouseGwList?brtcCode=11&pageNo=1",
                1,
                collectedAt,
                payload
        )));

        assertThat(snapshotRepository.findAll()).singleElement().satisfies(snapshot -> {
            assertThat(snapshot.getRawPayload()).isEqualTo(payload);
            assertThat(snapshot.getCollectedAt()).isEqualTo(collectedAt);
            assertThat(snapshot.getSource()).isEqualTo(ExternalDataSource.MYHOME_COMPLEX);
        });
    }

    @Test
    @DisplayName("같은 외부 응답을 다시 수집해도 원천 행을 중복 제거하지 않는다")
    void keepsRepeatedSnapshots() {
        ExternalDataSnapshot snapshot = ExternalDataSnapshot.create(
                ExternalDataSource.MYHOME_NOTICE,
                "rsdtRcritNtcList?suplyTy=01&pageNo=1",
                1,
                Instant.now(),
                "{}"
        );

        store.storeSnapshots(List.of(snapshot));
        store.storeSnapshots(List.of(ExternalDataSnapshot.create(
                snapshot.getSource(),
                snapshot.getRequestDescription(),
                snapshot.getPage(),
                snapshot.getCollectedAt().plusSeconds(1),
                snapshot.getRawPayload()
        )));

        assertThat(snapshotRepository.count()).isEqualTo(2);
    }

    @Test
    @DisplayName("외부 조회 실패 원인과 시각을 저장한다")
    void storesFailureReasonAndOccurredAt() {
        Instant occurredAt = Instant.parse("2026-08-23T00:01:00Z");
        store.storeFailure(ExternalDataCollectionFailure.create(
                ExternalDataSource.LH_LEASE_CATALOG,
                "lhLeaseInfo1/lhLeaseInfo1?page=1",
                occurredAt,
                "IllegalStateException",
                "원천 응답 오류"
        ));

        assertThat(failureRepository.findAll()).singleElement().satisfies(failure -> {
            assertThat(failure.getReason()).isEqualTo("원천 응답 오류");
            assertThat(failure.getOccurredAt()).isEqualTo(occurredAt);
        });
    }
}
