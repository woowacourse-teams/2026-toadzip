...
- LH 상세·공급의 성공 체크포인트는 마지막 실제 수집 성공 시각이다. 기본적으로 진행 중 공고는 6시간,
  종료 후 30일까지는 24시간마다 재수집하고 더 오래된 공고는 정기 수집에서 제외한다.
  날짜가 없거나 해석되지 않으면 진행 중 주기를 적용하며, 관리자 강제 갱신은 오래된 종료 공고도 조회한다.
- 유효한 성공 캐시를 다른 공고에 연결하는 작업은 체크포인트 시각을 연장하지 않는다.
...
현재 공통값 선택과 비활성 원천의 충돌은 [LH 정확성 검토의 M-02](lh-announcement-correctness-audit.md)에 남은 문제로 기록했다.
